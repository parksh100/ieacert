export default {
  methods: {
    ...
    // 미국은 Mar 15, 2022
    // 유럽은 15.05.2022
    // d = '20220315', f = 'YYYY.MM.DD' 날짜와 폼이 문자열
    // new Date() date객체로 들어올 수 있음.
    ...
    $convertNumberFormat(amount, format) {
      ...
      if (format.indexOf('.') === -1) {
        ...
        // 3500.1, '#,###.##' -> 3,500.1인 경우
        // format.length는 8, format.index(.) = 5 - 1 =2
      } else {
        // #.###,##
        groupingSeparator = '.'
        decimalSeparator = ','
      ...
      let sign = '' // amount가 음수로 들어왔을 때 // -3500, '#,###' -> -3,500
      let dec = 1
      ...
      // format = #,###.#0
      // value부분 parsefloat을 숫자로 들어왔든 문자로 들어왔든 숫자로 바꿈
